...
void generatePuzzle()
{
  ...
  nodes.push_back(Vector2i(rand()%N,rand()%N));// start from random node in grid

  while(!nodes.empty())
  {
    int n = rand()%nodes.size(); // choose random node from existing generated nodes
    Vector2i v = nodes[n];
    Vector2i d = DIR[rand()%4]; // choose random direction for next node

    if (cell(v).dirs.size()==3) {nodes.erase(nodes.begin() + n); continue;} // if already have 3 directions, skip adding new nodes
    if (cell(v).dirs.size()==2) if (rand()%50) continue;// 1/50 chance we have a 3 direction pipe, 49/50 chance we are satisfied with 2 direction pipe

    bool complete=1;
    for(auto D:DIR) // inspect all four directions
     if (!isOut(v+D) && cell(v+D).dirs.empty()) complete=0; // next node in dir good, and haven't generated any next node from cur node, continue
    if (complete) {nodes.erase(nodes.begin() + n); continue; } // skip generation if done

    if (isOut(v+d)) continue; // next node outside grid boundary, skip
    if (!cell(v+d).dirs.empty()) continue; // next node direction already generated
    cell(v).dirs.push_back(d); // set this node direction
    cell(v+d).dirs.push_back(-d); // set next node direction
    nodes.push_back(v+d); // enqueue next node
  }
}
